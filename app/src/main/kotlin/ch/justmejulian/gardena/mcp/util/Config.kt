 ...
 *
 * @param resourceBaseUrl canonical public base URL of this MCP server — must match exactly in token
 *   audience claims and the protected resource metadata document.
...
object Config {
   ...
   *
   * Both variables are optional — returns null when [MCP_RESOURCE_BASE_URL] is not set, which means
   * OAuth metadata endpoints will not be served (appropriate for stdio / local use).
   *
   * @return OAuthConfig, or null if [MCP_RESOURCE_BASE_URL] is not configured
   */
  fun loadOAuthConfig(): OAuthConfig? {
    val resourceBaseUrl =
      System.getenv(EnvVar.MCP_RESOURCE_BASE_URL.key)?.takeIf { it.isNotBlank() } ?: return null
